...
public abstract class AbstractEntityAssembler<T extends Entity> extends SlimefunItem implements EnergyNetComponent, TickingBlock {
    ...
    private void updateBlockInventory(BlockMenu menu, Block b) {
        if (!BlockStorage.hasBlockInfo(b) || BlockStorage.getLocationInfo(b.getLocation(), KEY_ENABLED) == null || BlockStorage.getLocationInfo(b.getLocation(), KEY_ENABLED).equals(String.valueOf(false))) {
            menu.replaceExistingItem(22, new CustomItem(Material.GUNPOWDER, "&7已启用: &4\u2718", "", "&e> 点击启用机器"));
            ...
            menu.replaceExistingItem(22, new CustomItem(Material.REDSTONE, "&7已启用: &2\u2714", "", "&e> 点击关闭机器"));
            menu.addMenuClickHandler(22, (p, slot, item, action) -> {
        ...
        menu.replaceExistingItem(31, new CustomItem(Material.PISTON, "&7调整: &3" + offset + "方块", "", "&f左键: &7+0.1", "&f右键: &7-0.1"));
    ...
    protected void constructMenu(BlockMenuPreset preset) {
        preset.addItem(1, new CustomItem(getHead(), "&7头颅槽", "", "&f这个槽位放头颅"), ChestMenuUtils.getEmptyClickHandler());
        preset.addItem(7, new CustomItem(getBody(), "&7组装原料槽", "", "&f这个槽位放组装原料"), ChestMenuUtils.getEmptyClickHandler());
        preset.addItem(13, new CustomItem(Material.CLOCK, "&7装配时间: &b30秒", "", "&f这个机器需要半分钟的时间装配", "&f请耐心等待!"), ChestMenuUtils.getEmptyClickHandler());
    }
